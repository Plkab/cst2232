...
**Exemple : UART2 avec DMA circulaire – Echo**

La première étape de l’implémentation consiste à configurer le module UART2. Les broches PA2 et PA3 sont configurées en AF7, permettant la communication série via USART2. Le registre BRR est réglé pour un débit de 115200 bps sur un bus APB1 à 84 MHz. Les bits TE, RE et UE sont activés pour permettre l’émission, la réception et activer le module UART. Ensuite, le DMA1 Stream6 est utilisé pour la transmission (TX) et le Stream5 pour la réception (RX). 

La réception est configurée en mode circulaire, ce qui signifie que les données reçues sont automatiquement stockées dans un buffer de taille fixe et que le DMA revient au début du buffer lorsqu’il atteint la fin. Cela permet une acquisition continue et non bloquante, essentielle pour des applications temps réel telles que la lecture de capteurs rapides ou l’échange de messages à haute fréquence. Le mode circulaire élimine le risque de perte de données, car le CPU n’a pas besoin de lire chaque octet immédiatement. 

Enfin, des fonctions pratiques permettent l’accès au buffer et l’émission via DMA. UART2_SendCharDMA et UART2_SendStringDMA permettent d’envoyer des caractères ou des chaînes de manière non bloquante, en remplissant le buffer TX et en déclenchant le DMA. Pour la réception, la fonction UART2_ReadChar lit les octets reçus à partir du buffer circulaire RX, en gérant automatiquement le pointeur de tête et le wrapping du buffer. 

Le programme principal illustre un simple echo, où chaque caractère reçu est renvoyé, démontrant la capacité du système à traiter les données reçues en continu sans bloquer le CPU. Cette approche garantit un transfert efficace et fiable pour des systèmes embarqués exigeants en temps réel.

```c
#include "stm32f4xx.h"

#define UART_TX_BUFFER_SIZE 256     // Taille du buffer TX (émission)
#define UART_RX_BUFFER_SIZE 256     // Taille du buffer RX (réception, circulaire)

uint8_t tx_buffer[UART_TX_BUFFER_SIZE]; // Buffer de transmission
uint8_t rx_buffer[UART_RX_BUFFER_SIZE]; // Buffer de réception (DMA circulaire)

// Pointeurs pour gestion du buffer circulaire RX 
volatile uint16_t rx_head = 0;  // Indice de lecture
volatile uint16_t rx_tail = 0;  // (peut servir si gestion avancée)

// Initialisation de l’UART2 (PA2=TX, PA3=RX)
void USART2_Init(uint32_t baudrate) {
    ...
    GPIOA->MODER |=  (2 << (2 * 2)) | (2 << (3 * 2));  // Mettre MODER2/3 = 10 (Alternate Function)
    GPIOA->AFR[0] &= ~((0xF << (4 * 2)) | (0xF << (4 * 3))); // Reset AFRL
    GPIOA->AFR[0] |= (7 << (4 * 2)) | (7 << (4 * 3));   // Sélectionner AF7 (USART2)

    // 3. Configurer le baudrate : BRR = fclk / baudrate
    // Ici fclk = 16 MHz, baudrate = 9600 -> BRR = ~1667 (0x0683)
    USART2->BRR = (16000000 / baudrate);

    // 4. Activer Transmission et Réception, puis activer l’USART
...
// Configuration DMA pour l’UART2 (TX et RX circulaire)      
/* Sur STM32F446 : 
   - USART2_TX = DMA1, Stream6, Channel4
   - USART2_RX = DMA1, Stream5, Channel4
*/
void DMA_UART2_Init(void) {
    // 1. Activer horloge DMA1 
    RCC->AHB1ENR |= RCC_AHB1ENR_DMA1EN;

    // TX DMA (Stream6, Channel4) 
    DMA1_Stream6->CR &= ~DMA_SxCR_EN;                  // Désactiver le DMA avant config
    DMA1_Stream6->PAR = (uint32_t)&USART2->DR;      // Destination : registre DR de l’UART
    DMA1_Stream6->M0AR = (uint32_t)tx_buffer;          // Source : buffer TX
    DMA1_Stream6->NDTR = 0;                            // Longueur à remplir plus tard
    DMA1_Stream6->CR = (4 << DMA_SxCR_CHSEL_Pos)       // Sélection Channel 4
                     | DMA_SxCR_MINC                   // Incrémentation mémoire
                     | DMA_SxCR_DIR_0;                 // Mémoire -> Périphérique
    DMA1_Stream6->CR |= DMA_SxCR_EN;                   // Activer le Stream6

    // RX DMA (Stream5, Channel4) 
    DMA1_Stream5->CR &= ~DMA_SxCR_EN;                  // Désactiver avant config
    DMA1_Stream5->PAR = (uint32_t)&USART2->DR;         // Source : registre DR
    DMA1_Stream5->M0AR = (uint32_t)rx_buffer;          // Destination : buffer RX
    DMA1_Stream5->NDTR = UART_RX_BUFFER_SIZE;          // Taille totale du buffer RX
    DMA1_Stream5->CR = (4 << DMA_SxCR_CHSEL_Pos)       // Channel 4
                     | DMA_SxCR_MINC                   // Incrément mémoire
                     | DMA_SxCR_CIRC;                  // Mode circulaire
    DMA1_Stream5->CR |= DMA_SxCR_EN;                  // Activer RX DMA en mode circulaire
}

// Envoi d’un caractère via UART2 + DMA 
void UART2_PrintCharDMA(char c) {
    tx_buffer[0] = c;              // Charger caractère dans le buffer
    DMA1_Stream6->NDTR = 1;        // Longueur = 1
    DMA1_Stream6->CR |= DMA_SxCR_EN; // Activer envoi
}

// Envoi d’une chaîne via DMA 
void UART2_PrintStringDMA(char* str) {
    uint16_t len = 0;
    while(str[len]) len++;         // Calcul de la longueur
    for(uint16_t i = 0; i < len; i++) {
        tx_buffer[i] = str[i];     // Copier la chaîne dans le buffer TX
    }
    DMA1_Stream6->NDTR = len;      // Longueur de l’envoi
    DMA1_Stream6->CR |= DMA_SxCR_EN; // Activer DMA pour TX
}

// Lecture asynchrone depuis le buffer circulaire RX 
uint8_t UART2_ReadChar(void) {
    uint8_t c = 0;
    // Vérifier si de nouvelles données sont arrivées
    if (rx_head != (UART_RX_BUFFER_SIZE - DMA1_Stream5->NDTR)) {
        c = rx_buffer[rx_head];                  // Lire caractère
        rx_head = (rx_head + 1) % UART_RX_BUFFER_SIZE; // Incrément circulaire
    }
    return c; // Retourne 0 si rien de nouveau
}

int main(void) {
    USART2_Init(9600);       // Init UART2 à 9600 bauds
    DMA_UART2_Init();        // Init DMA pour UART2

    UART2_PrintStringDMA("UART2 DMA Circulaire Ready\r\n");

    while(1) {
        uint8_t c = UART2_ReadChar();  // Lire caractère depuis RX
        if(c) {
            // Echo : renvoyer le même caractère reçu
            UART2_PrintCharDMA(c);
        }
    }
}
```
...
La configuration du DMA commence par le choix du stream et du canal correspondant à l’ADC1. Pour `DMA2_Stream0`, il faut d’abord désactiver le stream en réinitialisant le bit EN dans `DMA2_Stream0->CR`, afin de pouvoir modifier les registres de configuration en toute sécurité. On choisit ensuite le canal ADC1 via le registre `DMA2_Stream0->CR` en positionnant les bits CHSEL à `0 << 25`. 

Pour faciliter le transfert vers le tableau mémoire, il est nécessaire d’activer l’incrémentation automatique de la mémoire (`MINC`) en positionnant le bit correspondant dans `DMA2_Stream0->CR`. La taille des données source et destination est également configurée avec les bits `PSIZE` et `MSIZE`, typiquement à 16 bits pour un ADC 12 bits. Si un échantillonnage continu est souhaité, le mode circulaire est activé via le bit `CIRC` dans `DMA2_Stream0->CR`, ce qui permet au DMA de repartir au début du buffer une fois qu’il est rempli. Les adresses source et destination sont ensuite définies : `PAR = &ADC1->DR` pour la source (registre de données de l’ADC) et `M0AR = buffer` pour la mémoire. Le nombre de données à transférer est indiqué dans `NDTR = taille du buffer`. Enfin, une fois toutes ces configurations effectuées, le stream DMA est activé en positionnant le bit EN dans `DMA2_Stream0->CR`. Cette configuration permet un transfert continu et efficace des données ADC vers la RAM, réduisant la charge CPU et assurant un échantillonnage rapide et fiable.

Les registres du DMA utilisés pour l’acquisition des données de l’ADC sont essentiels pour contrôler le transfert automatique et surveiller son état. Le registre `DMA2_Stream0->CR` (Control Register) est le registre principal de configuration du flux DMA. Il permet de sélectionner le canal (`CHSEL`), d’activer l’incrémentation de la mémoire (`MINC`), de définir la taille des données source et destination (`PSIZE` et `MSIZE`), et d’activer le mode circulaire (`CIRC`) si nécessaire. C’est ce registre qui détermine le comportement global du DMA pour ce stream. Le registre `DMA2_Stream0->NDTR` (Number of Data Transfer Register) indique le nombre de données à transférer. 

Par exemple, si l’on souhaite lire trois canaux ADC, NDTR sera réglé sur 3. Chaque déclenchement du DMA décrémente ce compteur jusqu’à atteindre zéro, signalant la fin du transfert. Le registre `DMA2_Stream0->PAR` (Peripheral Address Register) contient l’adresse du périphérique source, ici `&ADC1->DR`, qui correspond au registre de données de l’ADC. C’est à partir de cette adresse que le DMA lit automatiquement les valeurs converties par l’ADC. Le registre `DMA2_Stream0->M0AR` (Memory 0 Address Register) spécifie l’adresse mémoire de destination, c’est-à-dire le buffer dans lequel seront stockées les données ADC. Grâce à l’incrémentation automatique (`MINC`), le DMA remplit séquentiellement le tableau sans intervention CPU. 

Enfin, les registres `DMA2->LISR` et DMA2->HISR sont utilisés pour surveiller le statut des flux DMA. Ils contiennent des flags indiquant la fin de transfert (`TCIF`), les erreurs de transfert (`TEIF`), ou encore la moitié de transfert (`HTIF`) dans le cas de mode double buffer. Ces flags permettent de déclencher des interruptions ou de vérifier que les données ont été correctement transférées.
